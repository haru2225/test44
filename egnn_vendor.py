...
def unsorted_segment_sum(data, segment_ids, num_segments):
    result = torch.zeros((num_segments, data.size(1)), device=data.device, dtype=data.dtype)
    ids = segment_ids.unsqueeze(-1).expand(-1, data.size(1))
    result.scatter_add_(0, ids, data)
    return result


def unsorted_segment_mean(data, segment_ids, num_segments):
    result = unsorted_segment_sum(data, segment_ids, num_segments)
    count = unsorted_segment_sum(torch.ones_like(data), segment_ids, num_segments)
